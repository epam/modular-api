...
DICT_WITH_CREDS_FOR_MOCK: dict[str, Any] = {
    'Arn': 'user/asdfasdf',
    'Credentials': {
...
def resolve_group_name(group_file: str) -> tuple:
    ...
    group_name = group_full_name_list[-1] \
        if isinstance(group_full_name_list, list) else group_full_name_list
    return group_full_name_list, group_name


def get_file_names_which_contains_admin_commands(
        path_to_scan: str,
) -> list[str]:
    listdir = [
        filename for filename in os.listdir(path_to_scan)
...
def _get_param_def_from_line(line: str) -> dict:
    if '\'\'' in line:
        line = line.replace('\'\'', '')
    split: list = [line for line in line.split('\'') if line]
    param_name: str | None = None
    param_doc: str | None = None
    alias_name: str | None = None
    param_type: str = 'str'
    is_path_to_file: bool = False
    file_extension = None
...
class CommandsDefinitionsExtractor:
    DEFAULT_METHOD = 'POST'
    DEFAULT_MOUNT_POINT = '/'
    click_to_our_types_mapping: dict[str, str] = {
        'choice': 'enum',
        'boolean': 'bool',
        'text': 'str',
        'integer': 'str'
    ...
    @staticmethod
    def _merge_route_configs(primary: dict, secondary: dict) -> dict:
        for node in secondary:
            if primary.get(node):
                secondary[node] = primary.get(node)
        return secondary

    def _get_default_route_config(
            self,
            group_full_name: str,
            command_name: str,
            subgroup: bool,
        ...
        if subgroup:
            ...
            resolved_parents = re.search(r'(\w+)(?=\.py)', module_path).group(0)
    ...
    @staticmethod
    def _get_route_configuration_from_line(
            line: str,
            default,
    ) -> dict[str, Any]:
    ...
    @staticmethod
    def _get_parameters_to_be_secured(line) -> list[str]:
        parameters_to_be_secured: list = []
    ...
    def _get_api_route_flag_and_secured_params(self, subgroup: bool) -> dict:
        ...
        for index, line in enumerate(lines):
            ...
            if '@{}.command'.format(self._group_name) in line:
                ...
                # analyze lines
                name: str | None = None
                is_command_hidden: bool = False  # hidden from help but executable
                is_command_disabled: bool = False  # completely unavailable
                route_config: dict = {}
                secure_parameters: list = []
                files_parameters: dict = {}
                security_parameters_found: bool = False
                secured_parameters_string: str = ''
                ...
                for line in prepared_lines:
                    ...
                    if '@click.option' in line:
                        ...
                        convert_content_to_file = \
                            response.get('convert_content_to_file')
                        temp_file_extension = \
                            response.get('temp_file_extension')
                    ...
                    # Keep backward compatibility with @shadow_wrapper
                    if '@shadow_wrapper' in line:
                        is_command_disabled = True  # Completely unavailable
                    if 'secured_params=' in line \
                        ...
                        if ']' in line:
                            ...
                            secure_parameters = \
                                self._get_parameters_to_be_secured(
                ...
                route_config = self._merge_route_configs(
    ...
    def extract(self, subgroup: bool) -> dict:
        ...
        for entity in dir(self._module):
            ...
            for param in click_command.params:
                ...
                if isinstance(param.type, (IntRange, FloatRange)):
                    ...
                    param_meta['description'] += \
                        f' {param.type.min or ""}<=x<={param.type.max or ""}'
        ...
        for name, body in definitions.items():
            command_config = routes_and_secured_params.get(name, {})
            is_command_disabled = command_config.pop('is_command_disabled', None)
